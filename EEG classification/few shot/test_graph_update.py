...
def Build_graph(data_dict, label_dict, epochs, mini_batch, lr, subject_num, kp, cl, kl, sl, dl, ml, restore_path, save_path, model, few, finetuning, data):
    ...
    with tf.Session(config=config) as sess:
        ...
        output = tf.get_default_graph().get_tensor_by_name("Graph_build/Graph_build/New_Model/Relation/output:0")

        with tf.variable_scope('Fine'):
            loss, train_step = cross_entropy(output, y, 0.00001)
        ...
        for i in range(len(key)):
            for re in range(10):
                ...
                for fine in range(fine_tuning_number):
                    sess.run(train_step,
                             feed_dict={x: tr, x_: tr_, y: tr_label, keep_prob: kp})
                ...
                for j in range(run):
                    ...
                    # sess.run(train_step, feed_dict={rep_sup: sup_feature, rep_que: que_feature, y: tr_label, keep_prob: kp, lrate: lr})
                    predic_value, train_accuracy, loss_print, train_prediction = sess.run([output, accuracy, loss, prediction], feed_dict={x: tr, x_: tr_, y: tr_label, keep_prob: 1.0})
