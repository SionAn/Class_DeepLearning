...
def Load_data(number):
    ...
    data_sort = np.zeros((count[0], 32, 1, 2), dtype=float)
    noise_sort = np.zeros((count[1], 32, 1, 2), dtype=float)
    label_data = np.zeros((count[0], 2))
    label_data[:, 0] = 1
    label_noise = np.zeros((count[1], 2))
    label_noise[:, 1] = 1
    sorted_num = 0
    unsorted_num = 0
    ...
    for i in range(threshold_number):
        if label[i, 0] == 1 and sorted_num < count[0]:
            data_sort[sorted_num, ::, ::, ::] = data_size[i, ::, ::, ::]
            sorted_num += 1
        if label[i, 1] == 1 and unsorted_num < count[1]:
            noise_sort[unsorted_num, ::, ::, ::] = data_size[i, ::, ::, ::]
            unsorted_num += 1

    return data_sort, label_data, noise_sort, label_noise
